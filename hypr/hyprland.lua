...
local fileManager = "ghostty > /dev/null && echo 'yazi'"
...
hl.config({
    general = {
        ...
        col = {
            ...
            inactive_border   = { colors = {"#000", "#000"}, angle = 45 },
        },
...
-------------------------------------
--- 
--- ONLY FOR APPS THAT STAY ON SAME WORKSPACES REGARDLESS THE DISPLAY SETUP!!

hl.window_rule({
	name = "rn-debugger-workspace",
	match = {
...
hl.on("hyprland.start", workspace_setup.run);
hl.on("config.reloaded", workspace_setup.run);
hl.on("monitor.added", workspace_setup.run);
hl.on("monitor.removed", workspace_setup.run);
